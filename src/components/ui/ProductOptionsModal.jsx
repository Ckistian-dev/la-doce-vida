...
const ProductOptionsModal = ({ product, isOpen, onClose, onAddToCart }) => {
  ...
  const [selectedWeight, setSelectedWeight] = useState(product?.minWeight || 500);
  ...
  const calculatedPrice = useMemo(() => {
    if (product?.type === 'by_weight') {
      return (selectedWeight / 1000) * product.pricePerKg;
    }
  ...
  const handleConfirm = () => {
    ...
    if (product.type === 'by_weight') {
      finalName = `${product.name} (${selectedWeight}g)`;
      finalPrice = calculatedPrice;
    }

    const itemToAdd = {
      ...product,
      id: `${product.id}-${Date.now()}`, // ID único para cada item customizado no carrinho
      name: finalName,
      price: finalPrice,
    };
        ...
        >
          ...
          <div className="mt-6 border-t pt-4 flex justify-between items-center">
            ...
            <button
              ...
              disabled={product.type === 'variations' && selectedVariations.length === 0}
